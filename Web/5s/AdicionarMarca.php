...
<table class="table" id="emp_table" border="0">
            <tr class="tr_header">
            <th scope="col">Nro Marca</th>
            <th scope="col">Nome Marca</th>
            </tr>
            <?php
            // count total number of rows
            $sql = "SELECT COUNT(*) AS cntrows FROM marcas";
            $result = mysqli_query($conexao,$sql);
            $fetchresult = mysqli_fetch_array($result);
            $allcount = $fetchresult['cntrows'];

            // selecting rows
            $sql = "SELECT * FROM marcas  ORDER BY ID ASC limit $row,".$rowperpage;
            $result = mysqli_query($conexao,$sql);
            $sno = $row + 1;
            while($fetch = mysqli_fetch_array($result)){
              $id_produto = $fetch['id'];
        $nroproduto = $fetch['Nome'];
                ?>
                <tr>
            ...
            <td>
            <a class="btn btn-danger btn-sm" style="color: #ffffff" href="Produtos/deletar_marca.php?id=<?php echo $id_produto ?>" role="button"><i class="far fa-trash-alt"></i>&nbsp;Excluir</a>
            </td>
                </tr>
            <?php
                $sno ++;
...
<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#AdicionarMarcas">
Adicionar
</button>
        </div>
...
<!-- Modal -->
<div class="modal fade" id="AdicionarMarcas" tabindex="-1" role="dialog" aria-labelledby="AdicionarMarcas" aria-hidden="true">
...
<form action="Entregas/_inserir_marca.php" method="post">
